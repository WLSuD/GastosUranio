<?php
	...
	class mainModel{
		...
		/*----------  Funcion ejecutar consultas simples  ----------*/
		public function ejecutar_consulta_simple_publica($consulta){
			$sql=self::conectar()->prepare($consulta);
			$sql->execute();
			return $sql;
		} /*--  Fin Funcion --*/
		...
		/*---------- Funcion datos tabla con condicion de un campo ----------*/
        public function datos_condicion1($tabla,$campo,$condicion){
			$tabla=self::limpiar_cadena($tabla);
			$campo=self::limpiar_cadena($campo);
			$condicion=self::limpiar_cadena($condicion);

			$sql=self::conectar()->prepare("SELECT * FROM $tabla WHERE $campo=:ID");
			$sql->bindParam(":ID",$condicion);
            $sql->execute();
		...
		/*---------- Funcion datos tabla con condicion de un campo ----------*/
		public function datos_condicion2($tabla,$campo1,$campo2,$condicion1,$condicion2){
			$tabla=self::limpiar_cadena($tabla);
			$campo1=self::limpiar_cadena($campo1);
			$campo2=self::limpiar_cadena($campo2);
			$condicion1=self::limpiar_cadena($condicion1);
			$condicion2=self::limpiar_cadena($condicion2);

			$sql=self::conectar()->prepare("SELECT * FROM $tabla WHERE $campo1 = :ID1 AND  $campo2 = :ID2");
			$sql->bindParam(":ID1",$condicion1);
			$sql->bindParam(":ID2",$condicion2);
			$sql->execute();
		...
		/*---------- Funcion modificar valor de un campo ----------*/
        public function cambio_estado($tabla,$columna,$valor,$campo,$condicion){
			$tabla		= self::limpiar_cadena($tabla);
			$columna 	= self::limpiar_cadena($columna);
			$valor		= self::limpiar_cadena($valor);
			$campo		= self::limpiar_cadena($campo);
			$condicion	= self::limpiar_cadena($condicion);

			$sql=self::conectar()->prepare("UPDATE $tabla SET $columna = :VALOR WHERE $campo = :ID");
		...
		/*---------- Funcion modificar saldo de programa  ----------*/
		public function cambio_saldo($tipo,$tabla,$columna,$valor,$campo,$condicion){
			$tipo		= self::limpiar_cadena($tipo);
			$tabla		= self::limpiar_cadena($tabla);
			$columna 	= self::limpiar_cadena($columna);
			$valor		= self::limpiar_cadena($valor);
			...
			if($tipo == "programar"){
				$sql=self::conectar()->prepare("UPDATE $tabla SET $columna = $columna - :VALOR WHERE $campo = :ID AND $columna >= :VALOR");
			}else{
				$sql=self::conectar()->prepare("UPDATE $tabla SET $columna = $columna + :VALOR WHERE $campo = :ID");
			}
		...
		/*---------- Funcion modificar estado de un campo a NULL ----------*/
		public function cambio_programa($tabla,$columna,$campo,$condicion){
			$tabla		= self::limpiar_cadena($tabla);
			$columna 	= self::limpiar_cadena($columna);
			$campo		= self::limpiar_cadena($campo);
			$condicion	= self::limpiar_cadena($condicion);

			$sql=self::conectar()->prepare("UPDATE $tabla SET $columna = NULL WHERE $tabla.$campo = :ID");
			$sql->bindParam(":ID",$condicion);
			$sql->execute();
